...
for ($i = 0; $i -lt $pipelines.Length; $i++) {
    ...
    Write-Host "$logPrefix Validating pipeline $($pipelines[$i])..."
    ...
    if (-not $?) {
        Write-Error "Content does not matched. Please run 'Update-OneGroundCDWorkflowsPaths.ps1' to update the pipelines."
        exit 1
    }
    Write-Host "$logPrefix $($pipelines[$i]) completed."
}

Write-Host "All pipelines validated."
