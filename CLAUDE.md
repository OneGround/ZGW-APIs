...
**Add an endpoint to an existing API version** (e.g. a new action on `OneGround.ZGW.Zaken`):
...
**Bump an API to a new minor version** (e.g. `v1.5` → `v1.6`):
...
**Add an EF Core migration** (after changing a `*.DataModel` entity or `DbContext`):
...
3. Review the generated file in `Migrations/` — check it doesn't silently drop/alter columns holding DataProtection-encrypted or HMAC-hashed data (see `docs/DATAPROTECTION.md`).
4. Do not hand-edit already-applied migrations; add a new one instead.

**Call another API from a handler** (inter-service, never direct DB access):
