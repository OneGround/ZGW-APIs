# OneGround Documenten API (DRC) - Standalone Deployment

This repository contains a standalone Docker Compose setup for the **OneGround Documenten API (DRC)**. This guide provides all the necessary steps to get the API running and connected to your own existing infrastructure.

## Table of Contents

- [OneGround Documenten API (DRC) - Standalone Deployment](#oneground-documenten-api-drc---standalone-deployment)
...
Your existing services should be accessible over a shared Docker network for the DRC container to connect to them.
...
[**Download the `standalone/DRC` directory**](https://download-directory.github.io/?url=https%3A%2F%2Fgithub.com%2FOneGround%2FZGW-APIs%2Ftree%2Fmain%2Fgetting-started%2F%2Fdocker-compose%2F%2Fstandalone%2F%2FDRC)
...
- `docker-compose.yml`: Defines the DRC service, maps the port, and handles network configuration.

---

## Choosing an Image Version

The `docker-compose.yml` file is configured to use a specific version of the `documenten-api` image (e.g., `1.5`). You can browse all available versions and select a different one if needed.

A complete list of published image versions can be found on the [**GitHub Packages page**](https://github.com/OneGround/ZGW-APIs/pkgs/container/documenten-api).
...
| `Auth__ValidAudience` | The audience value (`aud` claim) for validating JWTs. | `documenten-api` |
...
| `Services__ZRC__Api` | The base URL of the Zaken API (ZRC). | `http://zaken-api:5000` |
| `ZgwServiceAccounts__Credentials__0__Rsin` | The RSIN of the service account for inter-service communication. | `000000000` |
| `ZgwServiceAccounts__Credentials__0__ClientId` | The Client ID of the service account. | `drc-service-account` |
...
The DRC service must attach to the same Docker network as your other services to communicate with them.
...
networks:
  external_network:
    ...
    # where your Postgres, RabbitMQ, Redis, AC, BRC, ZTC, NRC and ZRC services are running.
...
If the startup is successful, the Documenten API will be running and accessible on port **5007** of your Docker host (e.g., `http://localhost:5007`). You can change this port mapping in the `docker-compose.yml` file if needed.
