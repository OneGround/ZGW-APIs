...
/// <summary>
/// Retained deliberately alongside <see cref="DomainToResponseRegister"/>, which is the Mapster port of
/// these same maps. BRC's controllers are migrated and use the register; three consumers outside the
/// controllers still map these types through AutoMapper and cannot be pointed at the register from here:
/// <list type="bullet">
///   <item><c>AuditTrailServiceBase.SetOld/SetNew&lt;TDto&gt;</c> (Common.Web) — every mutating handler.</item>
///   <item><c>RequestMerger.MergePartialUpdateToObjectRequest</c> (Common.Web) — PATCH /besluiten/{id}.</item>
///   <item><c>BesluitInformatieObjectenExpander</c> — GET /besluiten?expand=besluitinformatieobjecten.</item>
/// </list>
/// The two Common.Web consumers are shared by every service, and <c>IRequestMerger</c>'s signature exposes
/// AutoMapper's <c>IMappingOperationOptions</c>, so pointing them at Mapster is a change to shared
/// infrastructure rather than a per-service one. Delete this profile only once those consumers can use the
/// Mapster seam; <c>BrcMapperContractTests</c> fails if it goes early, and also pins the two mappers to the
/// same output for as long as both are live.
/// The request-to-domain and pagination profiles were controller-only and are already gone.
/// </summary>
