...
public class EnkelvoudigInformatieObjectBusinessRuleService : IEnkelvoudigInformatieObjectBusinessRuleService
{
    ...
    )
    {
        if (!string.IsNullOrEmpty(enkelvoudigInformatieObjectVersie.Identificatie))
        {
            bool existingEnkelvoudigInformatieObject = await _context2
        ...
        if (existingEnkelvoudigInformatieObjectId.HasValue)
        {
            ...
            */
            ...
            var existing = _context2
                .EnkelvoudigInformatieObjecten.AsNoTracking()
                .Include(e => e.EnkelvoudigInformatieObjectLock.GebruiksRechten)
                .Where(e => e.EnkelvoudigInformatieObjectLockId == existingEnkelvoudigInformatieObjectId.Value)
                .OrderBy(e => e.Versie)
                .Last();

            // Note: Locken en unlocken van documenten (drc-009)
            if (!existing.EnkelvoudigInformatieObjectLock.Locked)
            {
                var error = new ValidationError("status", ErrorCode.InvalidForReceived, "Een unlocked document mag niet bewerkt worden.");
            ...
            //if (string.IsNullOrEmpty(enkelvoudigInformatieObjectVersie.InformatieObject.Lock))
            if (string.IsNullOrEmpty(enkelvoudigInformatieObjectVersie.EnkelvoudigInformatieObjectLock.Lock))
            {
                ValidationError error;
                if (isPartialUpdate) // Note: To get Postman Tests get working
                {
            ...
            //if (existing.InformatieObject.Lock != enkelvoudigInformatieObjectVersie.InformatieObject.Lock)
            if (existing.EnkelvoudigInformatieObjectLock.Lock != enkelvoudigInformatieObjectVersie.EnkelvoudigInformatieObjectLock.Lock)
            {
                var error = new ValidationError("nonFieldErrors", ErrorCode.IncorrectLockId, "Incorrect lock ID.");
            ...
            else
            {
                ...
                // Het informatieobjecttype niet gewijzigd wordt
                else if (existing.InformatieObjectType != enkelvoudigInformatieObjectVersie.InformatieObjectType)
                {
                    var error = new ValidationError("informatieobjecttype", ErrorCode.UpdateNotAllowed, "Dit veld mag niet gewijzigd worden.");
            ...
            indicatiePutGebruiksRechtenValidionError =
                existing.EnkelvoudigInformatieObjectLock.GebruiksRechten.Count == 0
                && enkelvoudigInformatieObjectVersie.IndicatieGebruiksrecht.HasValue
                && enkelvoudigInformatieObjectVersie.IndicatieGebruiksrecht.Value;

            if (
                existing.EnkelvoudigInformatieObjectLock.GebruiksRechten.Count != 0
                && (
                    enkelvoudigInformatieObjectVersie.IndicatieGebruiksrecht == null
                    || !enkelvoudigInformatieObjectVersie.IndicatieGebruiksrecht.Value
                )
            )
            {
        ...
        else
        {
            ...
            indicatiePutGebruiksRechtenValidionError =
                enkelvoudigInformatieObjectVersie.IndicatieGebruiksrecht.HasValue && enkelvoudigInformatieObjectVersie.IndicatieGebruiksrecht.Value;
            ...
            */
            ...
            {
                // Note: Valideren informatieobjecttype op de EnkelvoudigInformatieObject - resource(drc - 001)
                await ValidateInformatieObjectTypeAsync(enkelvoudigInformatieObjectVersie.InformatieObjectType, errors);
            }
        }
