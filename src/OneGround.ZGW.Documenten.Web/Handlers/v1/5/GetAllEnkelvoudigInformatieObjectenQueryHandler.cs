        ...
        IRequestHandler<GetAllEnkelvoudigInformatieObjectenQuery, QueryResult<PagedResult<EnkelvoudigInformatieObject>>>
{
    ...
    //    return e =>
    //        (filter.Bronorganisatie == null || e.LatestEnkelvoudigInformatieObjectVersie.Bronorganisatie == filter.Bronorganisatie) // Note: Fixed issue not searching in all versions but latest only (GetAll returns latest versions only)
    //        && (filter.Identificatie == null || e.LatestEnkelvoudigInformatieObjectVersie.Identificatie == filter.Identificatie) // Note: Fixed issue not searching in all versions but latest only (GetAll returns latest versions only)
    ...
    //        ); // Note: Original version searches from Latest version only!
    //}
    // ---NEW VERSION ABOVE, OLD VERSION BELOW FOR REFERENCE---
