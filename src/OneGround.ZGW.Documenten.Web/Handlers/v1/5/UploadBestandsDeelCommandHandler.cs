        ...
        IRequestHandler<UploadBestandsDeelCommand, CommandResult<BestandsDeel>>
{
    ...
    public async Task<CommandResult<BestandsDeel>> Handle(UploadBestandsDeelCommand request, CancellationToken cancellationToken)
    {
        ...
        var enkelvoudigInformatieObject = bestandsdeel.EnkelvoudigInformatieObjectVersie.InformatieObject;

        try
        {
            using (var audittrail = _auditTrailFactory.Create(AuditTrailOptions, enkelvoudigInformatieObject.LegacyAuditTrail))
            {
                audittrail.SetOld<EnkelvoudigInformatieObjectGetResponseDto>(versie.InformatieObject);
                ...
                await using (var stream = request.Inhoud.OpenReadStream())
                {
                    ...
                    {
                        // No; record changes in audittrail so far and complete for now
                ...
                _logger.LogDebug(
                    "Uploaded successfully the last bestandsdeel and merged all bestansdelen and created document {documentUrn}.",
                    versie.Inhoud
                );
