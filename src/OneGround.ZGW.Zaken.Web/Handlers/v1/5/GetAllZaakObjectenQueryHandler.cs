        ...
        IRequestHandler<GetAllZaakObjectenQuery, QueryResult<PagedResult<ZaakObject>>>
{
    ...
    public async Task<QueryResult<PagedResult<ZaakObject>>> Handle(GetAllZaakObjectenQuery request, CancellationToken cancellationToken)
    {
        ...
        var pagedResult = await query
            .AsSplitQuery()
            .Include(z => z.Zaak)
            .Include(z => z.ObjectTypeOverigeDefinitie) // Note: Supported in >= v1.2
            .Include(z => z.Adres)
            .Include(z => z.Buurt)
    ...
    private Expression<Func<ZaakObject, bool>> GetZaakObjectFilterPredicate(Models.v1.GetAllZaakObjectenFilter filter)
    {
        return z =>
            (filter.Zaak == null || z.Zaak.Id == _uriService.GetId(filter.Zaak))
            && (!filter.ObjectType.HasValue || z.ObjectType == filter.ObjectType)
            &&
            // Note: the newly created COLLATION 'ci_collation': [provider = 'icu', locale = '@colStrength=secondary', deterministic = false] results in case-insensitive string comparison!
...
{
    public Models.v1.GetAllZaakObjectenFilter GetAllZaakObjectenFilter { get; internal set; }
