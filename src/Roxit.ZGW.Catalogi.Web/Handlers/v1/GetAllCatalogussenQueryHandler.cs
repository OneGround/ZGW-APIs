        ...
        IRequestHandler<GetAllCatalogussenQuery, QueryResult<PagedResult<Catalogus>>>
{
    ...
    private static Expression<Func<Catalogus, bool>> GetAllCatalogussenFilterPredicate(GetAllCatalogussenFilter filter)
    {
        return z =>
            (filter.Domein == null || z.Domein == filter.Domein)
            && (!filter.Domein__in.Any() || filter.Domein__in.Contains(z.Domein))
            && (filter.Rsin == null || z.Owner == filter.Rsin)
            && (!filter.Rsin__in.Any() || filter.Rsin__in.Contains(z.Owner));
    }
}

class GetAllCatalogussenQuery : IRequest<QueryResult<PagedResult<Catalogus>>>
{
    public GetAllCatalogussenFilter GetAllCatalogussenFilter { get; internal set; }
