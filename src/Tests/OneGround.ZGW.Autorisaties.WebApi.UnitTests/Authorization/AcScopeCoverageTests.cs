...
using Xunit;
using AnchorController = OneGround.ZGW.Autorisaties.Web.Controllers.v1.ApplicatiesController;

namespace OneGround.ZGW.Autorisaties.WebApi.UnitTests.Authorization;

/// <summary>
/// Every controller action in the Ac API must carry a scope attribute, or an explicit
...
/// </summary>
public class AcScopeCoverageTests
{
    [Fact]
    public void Every_controller_action_requires_a_scope_or_is_explicitly_exempt()
    {
        ...
        Assert.True(
            offenders.Count == 0,
            $"{offenders.Count} Ac controller action(s) carry neither a scope attribute nor "
        ...
        Assert.True(
            malformed.Count == 0,
            $"{malformed.Count} Ac exemption(s) carry an empty or whitespace reason, which the attribute's constructor "
