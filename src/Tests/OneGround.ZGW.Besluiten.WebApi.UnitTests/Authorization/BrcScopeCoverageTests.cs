...
using Xunit;
using AnchorController = OneGround.ZGW.Besluiten.Web.Controllers.v1.BesluitenController;

namespace OneGround.ZGW.Besluiten.WebApi.UnitTests.Authorization;

/// <summary>
/// Every controller action in the Brc API must carry a scope attribute, or an explicit
...
/// </summary>
public class BrcScopeCoverageTests
{
    [Fact]
    public void Every_controller_action_requires_a_scope_or_is_explicitly_exempt()
    {
        ...
        Assert.True(
            offenders.Count == 0,
            $"{offenders.Count} Brc controller action(s) carry neither a scope attribute nor "
        ...
        Assert.True(
            malformed.Count == 0,
            $"{malformed.Count} Brc exemption(s) carry an empty or whitespace reason, which the attribute's constructor "
