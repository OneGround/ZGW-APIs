...
using Xunit;
using AnchorController = OneGround.ZGW.Catalogi.Web.Controllers.v1.CatalogusController;

namespace OneGround.ZGW.Catalogi.WebApi.UnitTests.Authorization;

/// <summary>
/// Every controller action in the Ztc API must carry a scope attribute, or an explicit
...
/// </summary>
public class ZtcScopeCoverageTests
{
    [Fact]
    public void Every_controller_action_requires_a_scope_or_is_explicitly_exempt()
    {
        ...
        Assert.True(
            offenders.Count == 0,
            $"{offenders.Count} Ztc controller action(s) carry neither a scope attribute nor "
        ...
        Assert.True(
            malformed.Count == 0,
            $"{malformed.Count} Ztc exemption(s) carry an empty or whitespace reason, which the attribute's constructor "
