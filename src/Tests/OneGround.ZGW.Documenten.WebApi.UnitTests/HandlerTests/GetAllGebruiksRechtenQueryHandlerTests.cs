...
/// GetAllGebruiksRechtenQueryHandler (Handlers/v1/GetAllGebruiksRechtenQuery.cs).
///
...
/// critically — a GebruiksRecht whose EIO's LatestVersieId accidentally resolves to a
/// versie owned by another tenant can produce a false-positive auth result.
///
...
/// </summary>
public class GetAllGebruiksRechtenQueryHandlerTests
{
    ...
    /// (openbaar — would pass the auth threshold). Without v.Owner == _rsin in the subquery,
    /// the handler returns the GebruiksRecht because it finds RSIN_B's versie satisfying
    /// the auth check — a cross-tenant false positive. With the fix the subquery only
    /// matches versies owned by RSIN_A, so no versie is found and the GebruiksRecht
    /// is correctly excluded.
    /// </summary>
    [Fact]
    public async Task Handle_WithAuthorizationFilter_DoesNotMatchVersieFromDifferentOwner()
    {
        // Arrange
        ...
        ctx.GebruiksRechten.Add(
            new GebruiksRecht
            {
                Id = Guid.NewGuid(),
                InformatieObjectId = rsinAEioId,
                OmschrijvingVoorwaarden = "voorwaarden-A",
        ...
        var result = await handler.Handle(
            new GetAllGebruiksRechtenQuery { GetAllGebruiksRechtenFilter = new GetAllGebruiksRechtenFilter() },
        ...
        // The GebruiksRecht must NOT be returned.
        Assert.Empty(result.Result);
    }

    /// <summary>
    /// Happy-path: RSIN_A has an EIO with its own versie (openbaar, within auth threshold).
    /// When HasAllAuthorizations is false the handler must return that item and must NOT
    /// return RSIN_B's item (same typeX + openbaar).
    /// </summary>
    [Fact]
    public async Task Handle_WithAuthorizationFilter_ReturnsOnlyOwnTenantItems()
    {
        // Arrange: two tenants, each with EIO + versie(openbaar) + GebruiksRecht.
        using var ctx = BuildInMemoryDrcContext();
        ...
        Assert.All(result.Result, gr => Assert.Equal(RsinA, gr.InformatieObject.Owner));
    }
    ...
    {
        var options = new DbContextOptionsBuilder<DrcDbContext>().UseInMemoryDatabase(databaseName: $"drc-gebruiksrechten-{Guid.NewGuid()}").Options;
    ...
    /// <summary>
    /// Seeds EIO + versie (openbaar) + GebruiksRecht for both RSIN_A and RSIN_B with
        ...
        foreach (var rsin in new[] { RsinA, RsinB })
        {
            ...
            ctx.GebruiksRechten.Add(
                new GebruiksRecht
                {
                    ...
                    OmschrijvingVoorwaarden = $"voorwaarden-{rsin}",
                    Startdatum = DateTime.UtcNow,
                    CreationTime = DateTime.UtcNow,
                }
            );
    ...
    private static GetAllGebruiksRechtenQueryHandler BuildHandler(UnitTestDrcDbContext ctx, string currentRsin, bool hasAllAuthorizations)
    {
        ...
        var logger = Microsoft.Extensions.Logging.Abstractions.NullLogger<GetAllGebruiksRechtenQueryHandler>.Instance;
        var uriService = new Mock<IEntityUriService>();
        var documentKenmerkenResolver = new Mock<IDocumentKenmerkenResolver>();

        return new GetAllGebruiksRechtenQueryHandler(
            logger: logger,
