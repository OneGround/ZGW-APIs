...
public class DomainToResponseProfileTests : IDisposable
{
    ...
    {
        // Covers the v1.1-specific branch inside MapLatestEnkelvoudigInformatieObjectVersieResponse:
        // "Note: New in v1.1" style guard -- when BestandsDelen.Count != 0, Inhoud must be null
        // regardless of what the (mocked) uriService would otherwise return.
        var latestVersion = CreateVersion();
