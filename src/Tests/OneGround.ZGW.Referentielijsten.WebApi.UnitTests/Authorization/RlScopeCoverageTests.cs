...
using Xunit;
using AnchorController = OneGround.ZGW.Referentielijsten.Web.Controllers.ProcesTypenController;

namespace OneGround.ZGW.Referentielijsten.WebApi.UnitTests.Authorization;

/// <summary>
/// Every controller action in the Rl API must carry a scope attribute, or an explicit
...
/// </summary>
public class RlScopeCoverageTests
{
    [Fact]
    public void Every_controller_action_requires_a_scope_or_is_explicitly_exempt()
    {
        ...
        Assert.True(
            offenders.Count == 0,
            $"{offenders.Count} Rl controller action(s) carry neither a scope attribute nor "
        ...
        Assert.True(
            malformed.Count == 0,
            $"{malformed.Count} Rl exemption(s) carry an empty or whitespace reason, which the attribute's constructor "
                + $"rejects and a running host rejects while building its action descriptors: {string.Join(", ", malformed)}."
        );
    }

    [Fact]
    public void Every_controller_action_is_exempt_rather_than_scoped()
    {
        // RL serves static public reference-list data and registers no authentication, so its actions are
        // covered by [ScopeNotRequired] and never by a scope. An action here that demands a scope would be
        // unreachable: no client scope is resolvable on this host.
        var actions = ControllerActions();
        ...
        var scoped = actions
            .Where(a =>
                a.Action.IsDefined(typeof(BaseScopeAttribute), inherit: true) || a.Controller.IsDefined(typeof(BaseScopeAttribute), inherit: true)
            )
            .Select(a => $"{a.Controller.FullName}.{a.Action.Name}")
            .OrderBy(name => name, StringComparer.Ordinal)
            .ToList();
        ...
        Assert.True(
            scoped.Count == 0,
            $"RL actions are expected to be exempt, not scoped, but {scoped.Count} carry a scope attribute: {string.Join(", ", scoped)}."
        );
    }
