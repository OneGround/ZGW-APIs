# This script accepts the relative path to a certificate file as a command-line argument.
param(
    [Parameter(Mandatory=$true, HelpMessage="Please provide the relative path to the certificate file.")]
...
# Script Body
#
# This script installs the specified certificate into the
# Trusted Root Certification Authorities for the local machine.
# It must be run with Administrator privileges.
#----------------------------------------------------------------------

$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$certPath = Join-Path -Path $scriptDir -ChildPath $RelativeCertPath | Resolve-Path -Relative
...
if (-not (Test-Path $certPath)) {
    Write-Error "Certificate file not found at '$certPath'. Please check the path and try again."
} else {
    try {
